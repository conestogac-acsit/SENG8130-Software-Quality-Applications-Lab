...
describe('parseCsv - Duplicate Handling Logic', () => {
  ...
  beforeEach(() => {
    ...
    (globalThis as any).localStorageServiceInstance = storage;
  ...
  it('handles internal duplicates (Student)', async () => {
    const row = ['101', 'John', 'john@mail.com', 'A', 'G1', 'Student', '', '', 'Active', 'enrolled'].join(',');
    const csv = `${studentHeaders}\n${row}\n${row}`;
    const file = createFile(csv);

    const data = await parseCsv<Student>(file, 'Student');
    expect(data.length).toBe(2);
    const saved = storage.load<Student[]>('Student');
    expect(saved?.length).toBe(2);
  });

  it('handles existing duplicates (Evaluation skip)', async () => {
    storage.save('Evaluation', [
      {
        course: 'CS101',
        title: 'Midterm',
    ...
    const row = ['CS101', 'Midterm', 'Exam', '30', '2024-06-01', 'Dr. B', 'Main'].join(',');
    const csv = `${evaluationHeaders}\n${row}`;
    const file = createFile(csv);

    const data = await parseCsv<Evaluation>(file, 'Evaluation');
    expect(data.length).toBe(1);
    const saved = storage.load<Evaluation[]>('Evaluation');
    expect(saved?.[0].instructor).toBe('Dr. A');
  });

  it('saves when no duplicates are found', async () => {
    const row = ['102', 'Alice', 'alice@mail.com', 'B', 'G2', 'Student', '', '', 'Active', 'enrolled'].join(',');
    const csv = `${studentHeaders}\n${row}`;
    const file = createFile(csv);
  ...
  it('replaces duplicates when confirm is OK', async () => {
    storage.save('Evaluation', [
      {
        course: 'CS999',
        title: 'Special',
    ...
    const row = ['CS999', 'Special', 'Exam', '20', '2024-05-01', 'Dr. New', 'Main'].join(',');
    const csv = `${evaluationHeaders}\n${row}`;
    const file = createFile(csv);

    await parseCsv<Evaluation>(file, 'Evaluation');

    const saved = storage.load<Evaluation[]>('Evaluation');
    expect(saved?.[0].instructor).toBe('Dr. New');
  });

  it('skips duplicates when confirm is Cancel', async () => {
    storage.save('Evaluation', [
      {
        course: 'CS999',
        title: 'Special',
    ...
    const saved = storage.load<Evaluation[]>('Evaluation');
    expect(saved?.[0].instructor).toBe('Dr. Old');
  });
});
